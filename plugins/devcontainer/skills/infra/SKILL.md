---
name: infra
description: Design docker-compose simulations of the external resources an app depends on, for local/devcontainer development — relational/NoSQL databases (Postgres, MySQL, Oracle, MongoDB...), caches (Redis, DragonflyDB), object/blob storage (Azurite, LocalStack/MinIO), queues/streaming (RabbitMQ, Kafka, Redpanda), SMTP/outbound mail capture (Mailpit), inbound mailboxes (GreenMail IMAP/POP3), transactional-mail APIs (SES/SendGrid/Postmark), OAuth2/OIDC identity providers (mock-oauth2-server, Keycloak, Dex — including Entra ID/Okta/Auth0/Cognito stand-ins), SAML and LDAP directories, search indexes, vector databases, observability backends, feature-flag services, secrets managers, service discovery, API stubs, and beyond — each picked via a tier-0-vendor-sandbox/tier-1-identical/tier-2-substitute/tier-3-stub decision procedure with the version-gap or emulator status documented explicitly. Use when the user asks to "simulate <a database/queue/cache/storage/mail/SMTP/OAuth/login/SSO service> locally", "mock the identity provider", "catch outgoing emails", "get a real JWT for local dev", or when devcontainer:setup reaches its external-resources phase. Works for projects that already have a devcontainer, too.
---
