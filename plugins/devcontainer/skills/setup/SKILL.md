---
name: setup
description: Design and scaffold devcontainer(s) from scratch for a project that doesn't have one yet. When the project has a backend API and a frontend client as separate modules, gives each its own devcontainer by default — even when both share a language — and falls back to a single one only for full-stack frameworks or genuinely single-app projects. Analyzes each module's exact stack (language/version, package manager, existing formatter/linter config) to pick base images/features and wire matching IDE formatting settings, plus shared network topology, caches, env wiring, hot-reload, verification and decision docs. Use when the user asks to "set up a devcontainer", "add devcontainer support", "containerize the dev environment", "split the devcontainer into api and web", or to replicate how sibling/reference projects did theirs. Hands off to devcontainer:infra for simulated databases/queues/storage/mail/OAuth2 and to devcontainer:proxy for reverse-proxy simulation; day-to-day operation afterwards is devcontainer:workflow.
---
